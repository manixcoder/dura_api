    ...
    public function order_details_ByUser_id(Request $request)
    {
        ...
        else 
        {
            ...
            { 
                ...
                if($IsPresent>0)
                {
                    $IsPresent = DB::table('durapickupshedule')->where('user_id',$request->user_id)->where('driver_id','!=',0)->where('order_no','!=',1)->where('vehicle_id','!=',null)->orderby('id','desc')->get();
                    //print_r($IsPresent);die();
                    if($request ->status!="")
                    {
                        $IsPresent = DB::table('durapickupshedule')->where('user_id',$request->user_id)->where('status',$request ->status)->where('order_no','!=',1)->where('vehicle_id','!=',null)->orderby('id','desc')->get();    
                    }
                    ...
                    $totalPrice = $getpickup->finalprice;
                        ...
                        if($stopcount>0)
                        {
                            ...
                            $stoplocation[]     =array('stop_address1' => $getstop->stop_address1,
