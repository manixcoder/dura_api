    ...
    public function pickup_details(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                // print_r($nerarestDriver);
                // die;
                // echo "<pre>";
                // print_r($pickupsheduleData);
                // die;
...
/* multiple stop data start*/ 
                        ...
                        if ($stopcount > 0) {
                            ...
                            $driverlocationlink = 'https://www.google.com/maps/dir/' . $origin . '/' . $getstop->stoplat . ',' . $getstop->stoplon . '/' . $destination;
...
/* multiple stop data end*/
                        ...
                        $completePrice =  array(
                            'distance'      => $distance,
                            'kmprice'       => $distance * $getvehicle->kmfare,
    ...
    public function update_user_cart(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                if ($IsPresent) {
                    //  dd($request->qty);
                    $is_update = DB::table('cart_items')->where('user_id', $request->user_id)->where('item_id', $IsPresent->item_id)->Update(['qty' => $request->qty]);
                    return response()->json(['status' => 200, 'message' => 'Success', 'data' => $is_update], 200);
                } else {
